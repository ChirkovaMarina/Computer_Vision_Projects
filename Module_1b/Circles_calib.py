...
VIDEO_PATH = os.environ.get("CIRCLES_VIDEO", "calib/circles.mp4")
cap = cv2.VideoCapture(VIDEO_PATH)
if not cap.isOpened():
    raise FileNotFoundError(
        f"Cannot open video '{VIDEO_PATH}'. "
        "Set CIRCLES_VIDEO to a valid circles-grid calibration video path."
    )
found = 0
...
if not objpoints or not imgpoints:
    raise RuntimeError("Circle grid was not detected in any frame")

if found < 3:
    raise RuntimeError(f"Too few valid frames for calibration: {found}")
